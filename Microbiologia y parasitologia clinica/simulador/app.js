/* Simulador de examen — lógica de la app (sin dependencias externas) */
(function () {
  "use strict";

  // 🎨 PERSONALIZA: cambia el prefijo "studyKit" por algo único de esta asignatura
  // (p.ej. "farmaSimP1", "bioquimicaT2"...). Si no lo cambias y el alumno abre dos
  // simuladores de distintas asignaturas desde rutas de archivo muy parecidas,
  // podrían compartir localStorage por accidente.
  const STORAGE_STATS = "microParaClinica_stats_v1";
  const STORAGE_HISTORY = "microParaClinica_history_v1";
  const STORAGE_GAME = "microParaClinica_game_v1";
  const STORAGE_SRS = "microParaClinica_srs_v1";
  ...
  // 🎨 PERSONALIZA (muy recomendable, es un detalle que se nota mucho): pon títulos
  // de nivel con sabor a la asignatura, como se hizo para Farmacología ("Aprendiz de
  // Farmacognosia", "Técnico en Farmacocinética", "Doctor/a en Farmacología"...).
  // Los de aquí abajo son un fallback genérico que funciona para cualquier materia.
  const LEVELS = [
    { min: 0,    title: "Recolector/a de muestras",       icon: "🧫" },
    { min: 100,  title: "Aprendiz de tinción de Gram",     icon: "🔬" },
    { min: 250,  title: "Técnico/a de cultivo puro",       icon: "🧪" },
    { min: 500,  title: "Analista de antibiograma",        icon: "🦠" },
    { min: 800,  title: "Serólogo/a clínico/a",            icon: "🩸" },
    { min: 1200, title: "Cazador/a de parásitos",          icon: "🔎" },
    { min: 1700, title: "Especialista en Micro y Para",    icon: "🎓" },
    { min: 2300, title: "Doctor/a en Microbiología y Parasitología", icon: "👑" },
  ];
  ...
  const BADGE_DEFS = [
    ...
    { id: "repeated_expert", icon: "🔁", title: "Dominas lo que más se repite", desc: "Acierta 15 preguntas marcadas como muy repetidas.", check: () => countRepeatedMastered() >= 15 },
  ...
  function renderBossBanner() {
    ...
    if (!unlocked) {
      ...
      el.innerHTML = `
        ...
        <div class="bb-main">
          <div class="bb-title">Examen Final: El Gran Repaso</div>
      ...
      el.innerHTML = `
        <span class="bb-icon">👑</span>
        <div class="bb-main">
          <div class="bb-title">${already ? "¡Ya superaste el Examen Final!" : "Examen Final: El Gran Repaso"}</div>
          <div class="bb-desc">${already ? `Puedes repetirlo cuando quieras para mantener tu título de ${LEVELS[LEVELS.length - 1].title}.` : "Todo el temario mezclado, sin pistas y contrarreloj. Tu último gran repaso antes del examen real."}</div>
        </div>
